...
*xfer_set_offset_and_size = *Amanda::Xferc::xfer_set_offset_and_size;
...
*xfer_element_set_offset = *Amanda::Xferc::xfer_element_set_offset;
*xfer_element_set_size = *Amanda::Xferc::xfer_element_set_size;
*xfer_element_get_offset = *Amanda::Xferc::xfer_element_get_offset;
*xfer_element_get_orig_size = *Amanda::Xferc::xfer_element_get_orig_size;
*xfer_element_get_size = *Amanda::Xferc::xfer_element_get_size;
...
*xfer_source_shm_ring = *Amanda::Xferc::xfer_source_shm_ring;
*xfer_source_shm_ring_get_shm_name = *Amanda::Xferc::xfer_source_shm_ring_get_shm_name;
*xfer_source_file = *Amanda::Xferc::xfer_source_file;
...
*xfer_filter_crc = *Amanda::Xferc::xfer_filter_crc;
*xfer_filter_process = *Amanda::Xferc::xfer_filter_process;
*filter_process_get_err_fd = *Amanda::Xferc::filter_process_get_err_fd;
...
*xfer_dest_application = *Amanda::Xferc::xfer_dest_application;
*dest_application_get_err_fd = *Amanda::Xferc::dest_application_get_err_fd;
*dest_application_get_out_fd = *Amanda::Xferc::dest_application_get_out_fd;
*dest_application_get_dar_fd = *Amanda::Xferc::dest_application_get_dar_fd;
...
*XMSG_CHUNK_DONE = *Amanda::Xferc::XMSG_CHUNK_DONE;
*XMSG_CRC = *Amanda::Xferc::XMSG_CRC;
*XMSG_NO_SPACE = *Amanda::Xferc::XMSG_NO_SPACE;
*XMSG_SEGMENT_DONE = *Amanda::Xferc::XMSG_SEGMENT_DONE;
...
=head3 Amanda::Xfer::Dest::Holding (SERVER ONLY)
  ...
  Amanda::Xfer::Dest::Holding->new($max_memory);
...
To finish the last chunk:
  $mesg = $dest->finish_chunk();

if finish_chunk fail, $mesg is set to a string
...
sub xfer_status_to_value {
    my ($stringvalue) = @_;

    if (defined $_xfer_status_VALUES{$stringvalue}) {
	return $_xfer_status_VALUES{$stringvalue}
    }
...
#Convert an string to a enum value
sub xmsg_type_to_value {
    my ($stringvalue) = @_;

    if (defined $_xmsg_type_VALUES{$stringvalue}) {
	return $_xmsg_type_VALUES{$stringvalue}
    }
...
push @EXPORT_OK, qw($XMSG_CHUNK_DONE);
push @{$EXPORT_TAGS{"xmsg_type"}}, qw($XMSG_CHUNK_DONE);

$_xmsg_type_VALUES{"XMSG_CHUNK_DONE"} = $XMSG_CHUNK_DONE;

push @EXPORT_OK, qw($XMSG_CRC);
push @{$EXPORT_TAGS{"xmsg_type"}}, qw($XMSG_CRC);

$_xmsg_type_VALUES{"XMSG_CRC"} = $XMSG_CRC;

push @EXPORT_OK, qw($XMSG_NO_SPACE);
push @{$EXPORT_TAGS{"xmsg_type"}}, qw($XMSG_NO_SPACE);

$_xmsg_type_VALUES{"XMSG_NO_SPACE"} = $XMSG_NO_SPACE;

push @EXPORT_OK, qw($XMSG_SEGMENT_DONE);
push @{$EXPORT_TAGS{"xmsg_type"}}, qw($XMSG_SEGMENT_DONE);

$_xmsg_type_VALUES{"XMSG_SEGMENT_DONE"} = $XMSG_SEGMENT_DONE;
...
sub xfer_start_with_callback {
    ...
    Scalar::Util::weaken($cb);
    if (defined $cb) {
	my $releasing_cb = sub {
	    my ($src, $msg, $xfer1) = @_;
	    my $done = $msg->{'type'} == $XMSG_DONE;
	    $cb->(@_);
	    if ($done) { # break potential reference loop
		$xfer->{'src'} = undef;
		$xfer->{'cb'} = undef;
		$xfer = undef;
	    }
	};
	$xfer->{'xfer'}->get_source()->set_callback($releasing_cb);;
	$xfer->{'src'} = $xfer->{'xfer'}->get_source();
	$xfer->{'cb'} = $cb;
    ...
    $xfer->{'offset'} = $offset;
    $xfer->{'size'} = $size;
    xfer_start($xfer->{'xfer'}, $offset, $size);
}

sub xfer_set_offset_and_size {
    my ($xfer, $offset, $size) = @_;

    $xfer->{'offset'} = $offset if !defined $offset;
    $xfer->{'size'} = $size if !defined $size;
    xfer_set_offset_and_size($xfer->{'xfer'}, $offset, $size);
}
 ...
 *set_offset_and_size = *Amanda::Xfer::xfer_set_offset_and_size;
 ...
 *set_offset = *Amanda::Xfer::xfer_element_set_offset;
 *set_size = *Amanda::Xfer::xfer_element_set_size;
 *get_offset = *Amanda::Xfer::xfer_element_get_offset;
 *get_orig_size = *Amanda::Xfer::xfer_element_get_orig_size;
 *get_size = *Amanda::Xfer::xfer_element_get_size;
...
package Amanda::Xfer::Source::ShmRing;
...
#The C function adds the proper blessing -- this function
#just gets $pkg out of the way.
    Amanda::Xfer::xfer_source_shm_ring(@_);
}
 *get_shm_name = *Amanda::Xfer::xfer_source_shm_ring_get_shm_name;

package Amanda::Xfer::Source::File;
...
#The C function adds the proper blessing -- this function
#just gets $pkg out of the way.
    Amanda::Xfer::xfer_source_file(@_);
}
...
package Amanda::Xfer::Filter::Crc;
 ...
 *get_stderr_fd = *Amanda::Xfer::filter_process_get_err_fd;
...
package Amanda::Xfer::Dest::Application;
...
#The C function adds the proper blessing -- this function
#just gets $pkg out of the way.
    Amanda::Xfer::xfer_dest_application(@_);
}
 *get_dar_fd = *Amanda::Xfer::dest_application_get_dar_fd;
 *get_stderr_fd = *Amanda::Xfer::dest_application_get_err_fd;
 *get_stdout_fd = *Amanda::Xfer::dest_application_get_out_fd;
...
sub start {
    my $self = shift;
    $self->xfer_start_with_callback(@_);
}

sub set_offset_and_size {
    my $self = shift;
    $self->{'xfer'}->set_offset_and_size(@_);
}

sub get_source {
    my $self = shift;
    $self->{'xfer'}->get_source(@_);
}

sub get_status {
    my $self = shift;
    $self->{'xfer'}->get_status(@_);
}
